    ...
    function initPlugin() {
        ...
        function preloadImages() {
            userEffects.forEach(effect => {
                if (effect.type === 'image' && effect.imageData && !imageCache[effect.id]) {
                    const img = new Image();
                    img.src = effect.imageData;
                    imageCache[effect.id] = img;
                }
            });
        }

        function saveEffects() { localStorage.setItem('st_ambient_effects', JSON.stringify(userEffects)); preloadImages(); }
        ...
        class Particle {
            ...
            draw() {
                ...
                // 获取当前特效
                const effect = userEffects.find(e => e.id === config.type && e.type === 'image');
                if (effect && imageCache[effect.id] && imageCache[effect.id].complete) {
                    // 使用缓存的图片
                    ctx.drawImage(imageCache[effect.id], -this.size, -this.size, this.size * 2, this.size * 2);
                } else {

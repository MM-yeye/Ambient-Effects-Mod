// Ambient Effects - 简洁版（横向按钮 + 折叠特效库）
(function() {
    setTimeout(() => {
        try {
            initPlugin();
    ...
    function initPlugin() {
        ...
        function refreshEffectList() {
            ...
            if (allEffects.length === 0) {
                container.innerHTML = '<div class="ambient-desc">暂无特效</div>';
                return;
            }
            ...
            container.innerHTML = allEffects.map(effect => {
                ...
                const preview = isCss ? (effect.code ? effect.code.substring(0, 80) : '') : (isBuiltin ? '内置粒子特效' : '图片特效');
                    ...
                    <div style="border: 1px solid var(--SmartThemeBorderColor); border-radius: 6px; margin-bottom: 6px;">
                        <div class="effect-header" style="display: flex; justify-content: space-between; align-items: center; padding: 6px 10px; cursor: pointer; background: var(--SmartThemeBlurTintColor); border-radius: 6px;">
                            ...
                            <span style="display: flex; gap: 6px;">
                                <button class="menu_button use-effect" data-id="${effect.id}" data-source="${effect.source}" style="padding: 2px 6px; font-size: 12px;">使用</button>
                                ${deletable ? `<button class="menu_button red_button delete-effect" data-id="${effect.id}" data-source="${effect.source}" data-builtin-id="${effect.id}" style="padding: 2px 6px; font-size: 12px;">删除</button>` : ''}
                        ...
                        <div class="effect-body" style="display: none; padding: 8px; border-top: 1px solid var(--SmartThemeBorderColor);">
                            ${isCss ? `
                                ...
                                <button class="menu_button save-css" data-id="${effect.id}" style="padding: 3px 8px; font-size: 12px;">保存</button>
                            ` : `
            ...
            container.querySelectorAll('.use-effect').forEach(btn => {
                btn.addEventListener('click', (e) => {
                    ...
                    let effect;
                    if (source === 'builtin') effect = builtInEffects.find(e => e.id === id);
                    else effect = userEffects.find(e => e.id === id);
                    if (effect) {
